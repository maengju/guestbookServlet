...
public class GuestbookDAO {
	...
	public GuestbookDAO() {
		...
		try {
			...
			ds = (DataSource) ctx.lookup("java:comp/env/jdbc/oracle"); //Tomcat의 경우 java:comp/env/를 이름앞에 붙여줘야함
	...
	public List<GuestbookDTO> printAll(int startNum, int endNum){
		List<GuestbookDTO> list = new ArrayList<GuestbookDTO>();
		
		String sql = "select * from("
				+ "select rownum rn, tt.*from "
				+ "(select seq,name,email,homepage,subject,content, "
				+ "to_char(logtime, 'YYYY.MM.DD') as logtime from guestbook order by seq desc) tt"
				+ ") where rn>=? and rn<=?";
